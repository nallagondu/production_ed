...
3. Run these commands (they'll be customized for your account):

**Mac/Linux**:
```bash
# 1. Authenticate Docker to ECR (replace with your actual account ID and region from earlier)
...
# 3. Tag your image (replace with your actual account ID and region)
...
5. **Auto scaling**:
   - Minimum size: `0` (scales to zero when unused - saves money!)
...
1. Click on the **Default domain** URL (like: `abc123.YOUR-REGION.awsapprunner.com`)
   ...
   **Note**: The first request will take 30-60 seconds if the app scaled to zero. This is normal - it's "cold starting" your container. Subsequent requests will be fast!
...
With our minimal configuration (scaling to zero when unused):
- App Runner: ~$0.007/hour when running = ~$5/month if used 30 hours
- ECR: ~$0.10/GB/month for image storage
- Total: ~$1-5/month with light usage (scales to zero when idle!)

**Key savings**: With minimum instances set to 0, you only pay when someone actually uses your app!

### How to Save Money

1. **Delete when not using**: Stop your App Runner service
